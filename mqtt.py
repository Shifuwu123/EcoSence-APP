...
def on_message(client, userdata, msg):
    ...
    print(f"\n### Datos recibidos y guardados en {archivo}: {datos}")
...
def escribir_csv(archivo, encabezado, timestamp, datos, topico):
    ...
    with open(archivo, 'a', newline='') as f:
        writer = csv.writer(f)
        if topico == 'sensor/DHT11':
            writer.writerow([timestamp.split(' ')[0], timestamp.split(' ')[1], datos['temperatura'], datos['humedad']])
        elif topico == 'sensor/HD38':
            writer.writerow([timestamp.split(' ')[0], timestamp.split(' ')[1], datos['humedad_tierra']])
...
# Conectar al broker y suscribirse a todos los tópicos bajo 'sensor/'
